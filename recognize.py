# -*- coding: utf-8 -*-
"""
Created on Mon Nov 10 16:25:22 2018
...
tf.app.flags.DEFINE_integer('charset_size', 100, "Choose the first `charset_size` character to conduct our experiment.")
...
def build_graph(top_k):
    ...
    rate = tf.train.exponential_decay(2e-4, global_step, decay_steps=2000, decay_rate=0.96, staircase=True)
...
#        image_path = './test/00099/14762.png'
#        final_predict_val, final_predict_index = inference(image_path)
#        logger.info('the result info label {0} predict index {1} predict_val {2}'.format("099", final_predict_index,
#                                                                                         final_predict_val))
        image_path = './hcc/test1.png'
        final_predict_val, final_predict_index = inference(image_path)
        logger.info('the result predict index {0} predict_val {1}'.format(final_predict_index,final_predict_val))
